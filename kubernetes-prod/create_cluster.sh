...
KUBEADM_JOIN_CMD=$(echo $KUBEADM_INIT_TEXT | grep 'kubeadm join' -A 1)

echo $KUBEADM_JOIN_CMD > kubeadm_join.cmd

# Copy config
ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o ServerAliveInterval=30 yc-user@$MASTER_IP '
  mkdir -p $HOME/.kube \
